...
class linear_layer:
    ...
    def forward(self, X):
        ...
        # print(f"L1 out {forward_output.shape}")
        # print(f"N {X.shape[0]}")
    ...
    def backward(self, X, grad):
        ...
        self.gradient['b'] = np.dot(np.ones((1,X.shape[0])),grad)
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        backward_output = (1-np.power(np.tanh(X),2))*grad
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha <= 0.0:
                    ...
                    ###################################################################################################
                    vt = _alpha*momentum[module_name+ '_'+key] - _learning_rate*g

                    module.params[key] += vt
